...
player = ["Ólafur Finnbogi Ólafsson", "Sigurður Atli", "Jói berg", "Gunnar Sigga", "El chapo", "Yamz"]
...
def print_Main_Menu():
    ...
    while next != "2":
        print("Please select 2 for testing purpose")
        next = input("Selection: ")
    return next
...
def add_player_func(Current_menu):
    for i, element in enumerate(Current_menu[1:], 1):
        ...
        if element == "Captain":
            while tmp not in player:
                print(f"Player {tmp} not found in players list")
                print("Please select a valid player")
                for ind, member in enumerate(player, 1):
                    print(f"{ind}: {member}")
                tmp = input(f"Please enter {element}: ")
    ...
    clear_screen()
    dumme_menu_adding(Current_menu, "######  Team added successfully  ######")
    time.sleep(3)
def adding_team_function():
    clear_screen()
    print_Main_Menu()
    clear_screen()
    print_menu(teams_list)
    clear_screen()
    dumme_menu_adding(add_team)
    add_player_func(add_team)
    clear_screen()
