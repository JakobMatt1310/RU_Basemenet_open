...
class Association_UI:
    ...
    def print_associations(self):
        ...
        print()
        for i, ass in enumerate(all_associations, 1):
            print(f"{i}. {ass.name}")
        print()
            ...
            if return_command != "back":
                ...
                if selection.isdigit() == True:
                    ...
                    if selection <= len(all_associations):
                        ...
                        print("Invalid input")
                        association_to_change = None
                else:
                    print("Invalid input")
                    association_to_change = None
            else:
                return_command = ""
            if association_to_change != None:
                edit_info = edit_association_only_menu(association_to_change)
                if edit_info == 'b':
                    print("Going back")
                    return
                elif edit_info == "1":
                    return_command = self.change_association_name(association_to_change)
                elif edit_info == "2":
                    return_command = self.change_association_phone(association_to_change)
                elif edit_info == "3":
                    return_command = self.change_association_address(association_to_change)
